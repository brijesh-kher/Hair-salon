...
public class DatePicker extends AppCompatActivity implements TimePickerDialog.OnTimeSetListener {
    ...
    public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
        if((hourOfDay>9 && hourOfDay<14) || (hourOfDay>16 && hourOfDay<22))
        {
            if(minute==0)
            {
                ...
                DatabaseReference rootRef = FirebaseDatabase.getInstance().getReference().child("HAIRSTYLIST");
                        ...
                        for(DataSnapshot data : children) {
                            ...
                            if (Mail != null && Mail.equals(recievedEmail))
                            {
                                if (data.hasChild("Requests")) {
                                    QueryRef.orderByChild("Date And Time").equalTo(DateAndTime).addListenerForSingleValueEvent(new ValueEventListener() {
                                        @Override
                                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                            ...
                                            else
                                            {
                                              ...
                                              //  myref.child("CustomerName").setValue(re);
                                               ...
                                               // Toast.makeText(DatePicker.this,Date,Toast.LENGTH_SHORT).show();
                                                ...
                                                myref.child("Date And Time").setValue(DateAndTime);
                                                myref.child("Status of Booking").setValue("Deny");
    ...
    public void sendMail2()
    {
        ...
        MailAPI mailAPI = new MailAPI(DatePicker.this , mail2);
        mailAPI.execute();
    }
